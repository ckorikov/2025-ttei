...
struct GradFn
{
    ...
    GradFn(const std::initializer_list<std::shared_ptr<TensorImpl>> lst_)
        : inputs(lst_)
    {
    }
...
struct TensorImpl final
{
    ...
    explicit TensorImpl(std::initializer_list<int> shape_,
                        bool requires_grad_ = false)
        : ndims(shape_.size()), strides(shape_.begin(), shape_.end()),
          shape_(shape_.begin(), shape_.end()), requires_grad(requires_grad_)
    {
        ...
        {
            strides[i] =
                std::accumulate(std::next(shape_.begin(), i + 1), shape_.end(),
                                1, std::multiplies<size_t>());
        }
        strides.back() = 1;
    }

    explicit TensorImpl(std::initializer_list<size_t> shape_,
                        bool requires_grad_ = false)
        : ndims(shape_.size()), strides(shape_.begin(), shape_.end()),
          shape_(shape_.begin(), shape_.end()), requires_grad(requires_grad_)
    {
    ...
    explicit TensorImpl(std::vector<size_t> shape_, bool requires_grad_ = false)
        : ndims(shape_.size()), strides(shape_.begin(), shape_.end()),
          shape_(shape_.begin(), shape_.end()), requires_grad(requires_grad_)
    {
        int total_size = std::accumulate(shape_.begin(), shape_.end(), 1,
    ...
    void accumulate_grad(const std::vector<float> &grad_)
    {
        ...
        {
            std::transform(grad.begin(), grad.end(), grad_.begin(),
                           grad.begin(), std::plus<float>());
        }
    }
};
...
{
    AddOp(std::shared_ptr<TensorImpl> a, std::shared_ptr<TensorImpl> b)
        : GradFn({a, b})
    {
    }
...
{
    MulOp(std::shared_ptr<TensorImpl> a, std::shared_ptr<TensorImpl> b)
        : GradFn({a, b})
    {
    }
    ...
    { // c = a * b, ∂c/∂a = b, ∂c/∂b = a
        ...
        std::transform(final_grad_a.begin(), final_grad_a.end(),
                       output.grad.begin(), final_grad_a.begin(),
                       std::multiplies<float>());
        std::transform(final_grad_b.begin(), final_grad_b.end(),
                       output.grad.begin(), final_grad_b.begin(),
                       std::multiplies<float>());
...
{
    SubOp(std::shared_ptr<TensorImpl> a, std::shared_ptr<TensorImpl> b)
        : GradFn({a, b})
    {
    }
...
{
    MaxOp(std::shared_ptr<TensorImpl> a, std::shared_ptr<TensorImpl> b)
        : GradFn({a, b})
    {
    }
    ...
    void backward(TensorImpl &output) override
    {
        ...
        std::transform(
            inputs[0]->data.begin(), inputs[0]->data.end(),
            inputs[1]->data.begin(), final_grad_a.begin(),
            // [](float a_, float b_){ if (std::abs(a_ - b_) < 1e-8) { return
            // 0.5f; } else { return a_ > b_ ? 1.0f : 0.0f; } }
            [](float a_, float b_) { return a_ > b_ ? 1.0f : 0.0f; });

        std::transform(
            inputs[0]->data.begin(), inputs[0]->data.end(),
            inputs[1]->data.begin(), final_grad_b.begin(),
            // [](float a_, float b_){ if (std::abs(a_ - b_) < 1e-8) { return
            // 0.5f; } else { return a_ > b_ ? 1.0f : 0.0f; } }
...
struct UnaryMinusOp final : public GradFn
{
    ...
    { // c = -a, ∂c/∂a = -1
        ...
        std::transform(final_grad.begin(), final_grad.end(),
                       output.grad.begin(), final_grad.begin(),
                       std::multiplies<float>());
...
struct SqrtOp final : public GradFn
{
    ...
    void backward(TensorImpl &output) override
    { // c = sqrt(a), ∂c/∂a = 1 / (2 * sqrt(a) + 1e-8)
        std::vector<float> final_grad(output.data.size());
        std::transform(inputs[0]->data.begin(), inputs[0]->data.end(),
                       final_grad.begin(), [](float t)
                       { return 1.0f / (2.0 * std::sqrt(t) + 1e-8); });
...
struct LogOp final : public GradFn
{
    ...
    void backward(TensorImpl &output) override
    { // c = log(a), ∂c/∂a = 1 / ( a + 1e-8)
        std::vector<float> final_grad(output.data.size());
        std::transform(inputs[0]->data.begin(), inputs[0]->data.end(),
                       final_grad.begin(),
                       [](float t) { return 1.0f / (t + 1e-8); });
...
{
    DivOp(std::shared_ptr<TensorImpl> a, std::shared_ptr<TensorImpl> b)
        : GradFn({a, b})
    {
    }

    void backward(TensorImpl &output) override
    { // c = a / b, ∂c/∂b = -a / (b^2 + 1e-8), ∂c/∂a = 1 / b
...
{
    MatrixMatrixProdOp(const std::shared_ptr<TensorImpl> &a,
    ...
    void backward(TensorImpl &output) override;
    std::string typestr() const noexcept override
    {
        return "MatrixMatrixProdOp";
    }
};

struct MatrixVectorProdOp final : public GradFn
{
    MatrixVectorProdOp(const std::shared_ptr<TensorImpl> &a,
                       const std::shared_ptr<TensorImpl> &b)
        : GradFn({a, b})
    {
    }
...
struct BroadcastOp : public GradFn
{
    ...
    // BroadcastOp(const std::vector<size_t>& input_shape_, const
    // std::vector<size_t>& target_shape_)
    BroadcastOp(std::shared_ptr<TensorImpl> a,
...
class Tensor final
{
  ...
  public:
    Tensor(bool requires_grad_ = false) : impl_(new TensorImpl)
    {
        impl_->requires_grad = requires_grad_;
    }

    Tensor(const std::vector<float> &data_, bool requires_grad_ = false)
    ...
    Tensor(const std::vector<float> &data_, std::vector<size_t> shape,
           bool requires_grad_ = false)
        : impl_(new TensorImpl)
    {
        impl_->data = data_;
        ...
        {
            impl_->strides[i] =
                std::accumulate(std::next(shape.begin(), i + 1), shape.end(), 1,
                                std::multiplies<size_t>());
        }
        impl_->strides.back() = 1;
    }

    Tensor(std::initializer_list<float> data_, bool requires_grad_ = false)
        : impl_(new TensorImpl)
    {
    ...
    Tensor(std::initializer_list<int> shape_, bool requires_grad_ = false)
        : impl_(new TensorImpl(shape_, requires_grad_))
    {
    }

    Tensor(std::initializer_list<size_t> shape_, bool requires_grad_ = false)
        : impl_(new TensorImpl(shape_, requires_grad_))
    {
    }

    Tensor(std::vector<size_t> shape_, bool requires_grad_ = false)
        : impl_(new TensorImpl(shape_, requires_grad_))
    {
    }
    ...
    void init(std::initializer_list<float> data_)
    {
        if (!empty() && data_.size() != size())
        {
            throw std::invalid_argument(
                "Can't initialize tensor of size " + std::to_string(size()) +
                " with data which size is " + std::to_string(data_.size()));
        }
        impl_->data = data_;
    }
    ...
    void set_grad(std::shared_ptr<GradFn> node) { impl_->grad_fn = node; }
  ...
  public:
    ...
    void requires_grad(bool new_state_) noexcept
    {
        impl_->requires_grad = new_state_;
    }
    ...
    {
        size_t new_size = std::accumulate(new_shape.begin(), new_shape.end(), 1,
                                          std::multiplies<size_t>());
        if (empty())
        {
        ...
        if (size() != new_size)
        {
            throw std::invalid_argument("Invalid shape for tensor of size " +
                                        std::to_string(size()));
        }
        ...
        {
            impl_->strides[i] = std::accumulate(
                std::next(impl_->shape_.begin(), i + 1), impl_->shape_.end(), 1,
                std::multiplies<size_t>());
        }
    }
    ...
    Tensor &operator+=(const Tensor &rhs)
    {
        if (impl_->requires_grad || rhs.impl_->requires_grad)
        {
            throw std::runtime_error(
                "A variable requires grad is used in a in-place += operation.");
        }
        if (size() != rhs.size() ||
            !std::equal(impl_->shape_.begin(), impl_->shape_.end(),
                        rhs.impl_->shape_.begin()))
        {
            throw std::invalid_argument(
                "Can't apply operator+=() for tensors of shape " +
                vector_to_string(impl_->shape_) + " and " +
                vector_to_string(rhs.impl_->shape_));
        }

        std::cout << "impl_->requires_grad || rhs.impl_->requires_grad = "
                  << impl_->requires_grad ||
            rhs.impl_->requires_grad;
    ...
    Tensor &operator-=(const Tensor &rhs)
    {
                        ...
                        rhs.impl_->shape_.begin()))
        {
            throw std::invalid_argument(
                "Can't apply operator-=() for tensors of shape " +
                std::to_string(size()) + " and " + std::to_string(rhs.size()));
        }
  ...
  private:
    size_t compute_index(const std::initializer_list<size_t> &idx) const
    {
        if (std::equal(impl_->shape_.begin(), impl_->shape_.end(), idx.begin(),
                       std::less<float>()))
        {
            throw std::invalid_argument("Invalid indexes for tensor of shape" +
                                        vector_to_string(impl_->shape_));
        }

        return std::inner_product(impl_->strides.begin(), impl_->strides.end(),
                                  idx.begin(), 0);
    }
  ...
  public:
...
#endif
        return std::accumulate(impl_->shape_.begin(), impl_->shape_.end(), 1,
                               std::multiplies<size_t>());
    }
    ...
    void fill(float arg)
    {
        std::fill(impl_->data.begin(), impl_->data.end(), arg);
    }
    ...
    Tensor operator+(const Tensor &rhs)
    {
                        ...
                        rhs.impl_->shape_.begin()))
        {
            throw std::invalid_argument(
                "Can't apply operator+() for tensors of shape (" +
                std::to_string(impl_->shape_[0]) + ", " +
                std::to_string(impl_->shape_[1]) + ") and (" +
    ...
    Tensor operator-()
    {
        Tensor tmp;
        tmp.impl_ = std::make_shared<TensorImpl>(*impl_);
        std::transform(tmp.impl_->data.begin(), tmp.impl_->data.end(),
                       tmp.impl_->data.begin(), [](float t) { return -t; });
    ...
    static Tensor dot_product(const Tensor &a, const Tensor &b)
    {
        ...
        Tensor c({a.shape()[0]});
        std::transform(a.impl_->data.begin(), a.impl_->data.end(),
                       b.impl_->data.begin(), c.impl_->data.begin(),
                       std::multiplies<float>());
        return c;
    }
    ...
    Tensor transpose()
    {
        ...
        {
            throw std::invalid_argument(
                "Invalid tensor shape for transpose: (" +
    ...
    static Tensor matrix_matrix_product(const Tensor &a, const Tensor &b)
    {
        ...
        {
            throw std::invalid_argument(
                "Invalid shapes for matmul: (" + std::to_string(M) + ", " +
                std::to_string(N) + ") and (" + std::to_string(b.shape()[0]) +
                ", " + std::to_string(K) + ").");
        }
        Tensor c({M, K});
    ...
    Tensor broadcast_to(const std::vector<size_t> &target_shape) const
    {
        ...
        {
            throw std::invalid_argument(
                "Can't broadcast tensor with ndims > 1");
        }
        size_t new_sz = std::reduce(target_shape.begin(), target_shape.end(), 1,
                                    std::multiplies<size_t>());
        if (new_sz < size())
        {
            throw std::invalid_argument("Can't broadcast tensor from size " +
                                        std::to_string(size()) + " to size " +
                                        std::to_string(new_sz));
        }
        ...
        {
            auto grad_fn =
                std::make_shared<BroadcastOp>(impl_, shape(), target_shape);
    ...
    static Tensor matrix_vector_product(const Tensor &a, const Tensor &b)
    {
        ...
        {
            throw std::invalid_argument(
                "Invalid arguments shape for matrix_vector_product: (" +
                std::to_string(a.shape()[0]) + ", " +
                std::to_string(a.shape()[1]) + ") and (" +
                std::to_string(b.shape()[0]) + ").");
        }
    ...
    static Tensor max(const Tensor &a, const Tensor &b)
    {
        ...
        std::transform(tmp.impl_->data.begin(), tmp.impl_->data.end(),
                       b.impl_->data.begin(), tmp.impl_->data.begin(),
    ...
    Tensor sum(size_t axis) const
    {
        ...
        {
            throw std::invalid_argument(
                "Can't apply sum() for tensor with ndims > 2");
        }
        if (axis > size())
        {
            throw std::invalid_argument("Can't find axis " +
                                        std::to_string(axis) + " in tensor");
        }
...
void MatrixMatrixProdOp::backward(TensorImpl &output)
{ // C = A @ B.
    Tensor grad_a(inputs[0]->data.size());
    Tensor grad_b(inputs[1]->data.size());

    grad_a = Tensor::matmul(Tensor(output.grad, output.shape_, true),
                            Tensor(inputs[1], true).transpose());
    grad_b = Tensor::matmul(Tensor(inputs[0], true).transpose(),
                            Tensor(output.grad, output.shape_, true));
...
void MatrixVectorProdOp::backward(TensorImpl &output)
{ // C = A @ b.
    Tensor grad_a(inputs[0]->data.size());
    Tensor grad_b(inputs[1]->data.size());

    grad_a =
        Tensor::matmul(Tensor(output), Tensor(inputs[1], true).transpose());
    grad_b =
        Tensor::matmul(Tensor(inputs[0], true).transpose(), Tensor(output));
...
Tensor reduce_sum_to_shape(const Tensor &input,
...
struct Linear : Layer
{
    ...
    Linear(size_t in_features, size_t out_features)
        : weight({in_features, out_features}, true), bias({out_features}, true)
    {
    ...
    void forward(const Tensor &input, Tensor &output) override
    {
        ...
        if (input.ndims() == 2)
        {
            output = Tensor::matmul(input, weight) +
                     bias.broadcast_to({input.shape()[0], out_features});
        }
        else
        {
    ...
    {
        std::stringstream ss;
        ss << "Linear(in_features=" << weight.dim(0)
           << ", out_features=" << weight.dim(1) << ")";
        return ss.str();
    }
};
...
struct Model
{
    ...
    void backward(const Tensor &output) { output.backward(); }
