...
public:
    ...
    std::vector<float> classic_multiply(const std::vector<float>& A, const std::vector<float>& B, int rowsA, int colsA, int colsB) {
    ...
    std::vector<float> strassen_multiply(const std::vector<float>& A, const std::vector<float>& B, int n) {
        if (n == 2) {
        ...
        for (int i = 0; i < mid; ++i) {
            for (int j = 0; j < mid; ++j) {
                int index = i * mid + j;
                result[i * n + j] = M5[index] + M4[index] - M2[index] + M6[index];
                result[i * n + j + mid] = M1[index] + M2[index];
                result[(i + mid) * n + j] = M3[index] + M4[index];
                result[(i + mid) * n + j + mid] = M5[index] + M1[index] - M3[index] - M7[index];
            }
        }
    ...
    // Метод Винограда
    std::vector<float> winograd_multiply(const std::vector<float>& A, const std::vector<float>& B, int rowsA, int colsA, int colsB) {
        if (A.size() != rowsA * colsA || B.size() != colsA * colsB) {
            throw std::invalid_argument("Invalid matrix dimensions for multiplication");
        }
        ...
        for (int j = 0; j < colsB; ++j) {
            for (int k = 0; k < colsA / 2; ++k) {
                colFactor[j] += B[2 * k * colsB + j] * B[(2 * k + 1) * colsB + j];
            }
        }
        ...
        for (int i = 0; i < rowsA; ++i) {
            for (int j = 0; j < colsB; ++j) {
                ...
                for (int k = 0; k < colsA / 2; ++k) {
                    result[i * colsB + j] += (A[i * colsA + 2 * k] + B[(2 * k + 1) * colsB + j]) *
        ...
        if (colsA % 2) {
            for (int i = 0; i < rowsA; ++i) {
                for (int j = 0; j < colsB; ++j) {
                    result[i * colsB + j] += A[i * colsA + colsA - 1] * B[(colsA - 1) * colsB + j];
                }
            }
        }
    ...
    std::vector<float> auto_multiply(const std::vector<float>& A, const std::vector<float>& B, int rowsA, int colsA, int colsB) {
        if (A.size() != rowsA * colsA || B.size() != colsA * colsB) {
            throw std::invalid_argument("Invalid matrix dimensions for multiplication");
        }
...
private:
    ...
    std::vector<float> strassen_multiply_2x2(const std::vector<float>& A, const std::vector<float>& B) {
